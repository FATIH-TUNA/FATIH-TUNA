...
while True:
    rahmen=time.time()
    ret,camera=pen.read()
    results=model(camera,stream=True)
    for k in results:
        boxes=k.boxes
        for box in boxes:
            ...
            cls=int(box.cls[0])
            cv2.rectangle(camera,(x1,y1),(w+x1,h+y1),(0,100,200),thickness=2)
            cv2.putText(camera,classNames[classNamess[cls]],(x1,y1),cv2.FONT_HERSHEY_SIMPLEX,1,(200,100,0),thickness=2)
